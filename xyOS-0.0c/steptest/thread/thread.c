...
int init_task_tty (void)
{
	int i;
	list_init (&all_list);
	list_init (&ready_list);
	
	i = init_thread (&tty_task, 4, "tty_task", (u32 *)task_tty);
	list_insert_by_pri (&tty_task.elem, &ready_list);		
	list_insert_back (&tty_task.all_elem, &all_list);
	return i;
...
int init_second_thread (void)
{
	...
	i = init_thread (&sec_thread, 4, "testB", (u32 *)testB);
...
int init_third_thread (void)
{
	...
	i = init_thread (&thi_thread, 4, "testC", (u32 *)testC);
...
init_K_thread (struct task_struct *thread, int priority, const char *name, u32 *function)
{
	thread->regs.cs = __KERNEL_CS;
	thread->regs.gs = __KERNEL_DS;
	thread->regs.fs = __KERNEL_DS;
	thread->regs.es = __KERNEL_DS;
	thread->regs.ds = __KERNEL_DS;
	thread->regs.ss = __KERNEL_DS;
